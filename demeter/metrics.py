...
NUMERIC_COLUMNS = [
    "DAP_cm", "Altura_m", "Volume_m3", "Coord_X", "Coord_Y",
    "Idade_anos", "DensidadeMadeira_t_m3",
]
...
def coerce_numeric(df: pd.DataFrame, column: str) -> None:
    if column in df.columns:
        df[column] = pd.to_numeric(df[column].map(_normalize_number), errors="coerce")


def prepare_inventory(df: pd.DataFrame, form_factor: float = 0.42) -> pd.DataFrame:
    df = df.copy()
    ...
    df["Talhao"] = df["Talhao"].fillna("Não informado").astype(str)

    if "Especie" not in df.columns:
        df["Especie"] = "Não informada"
    df["Especie"] = df["Especie"].fillna("Não informada").astype(str)

    if "Parcela" not in df.columns:
        df["Parcela"] = "Sem parcela"
    df["Parcela"] = df["Parcela"].fillna("Sem parcela").astype(str)

    if "Num_Arvore" not in df.columns:
        df["Num_Arvore"] = np.arange(1, len(df) + 1)

    if "DAP_cm" in df.columns:
        df["AreaBasal_m2"] = math.pi * np.square(df["DAP_cm"] / 200)
    else:
    ...
    if {"DAP_cm", "Altura_m"}.issubset(df.columns):
        estimated = df["AreaBasal_m2"] * df["Altura_m"] * float(form_factor or 0.42)
        df["Volume_Estimado_m3"] = estimated
...
def kpi_summary(df):
    ...
    return {
        ...
        "area_basal_total": total("AreaBasal_m2"),
    }
...
def stand_summary(df):
    if df.empty:
        return pd.DataFrame(columns=[
            "Talhao", "Arvores", "DAP_medio", "Altura_media",
            "Volume_m3", "Carbono_tC", "CO2e_t",
        ])
    return (
        df.groupby("Talhao", dropna=False)
        .agg(
...
def species_summary(df):
    ...
    return (
        df.groupby("Especie", dropna=False)
        .agg(
            Arvores=("Num_Arvore", "count"),
            Volume_m3=("Volume_m3", "sum"),
            Carbono_tC=("Carbono_tC", "sum"),
