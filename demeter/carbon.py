...
def enrich_carbon(
    ...
    project_area_ha=10,
    sample_area_ha=1,
    default_density=0.50,
    carbon_fraction=0.47,
    bef=1.20,
    root_ratio=0.24,
):
    df = df.copy()
    expansion_factor = project_area_ha / max(sample_area_ha, 0.000001)
    ...
    df["BiomassaAcimaSolo_t"] = df["BiomassaFuste_t"] * bef
    df["BiomassaRaiz_t"] = df["BiomassaAcimaSolo_t"] * root_ratio
    df["BiomassaTotal_t"] = df["BiomassaAcimaSolo_t"] + df["BiomassaRaiz_t"]
    df["Carbono_tC"] = df["BiomassaTotal_t"] * carbon_fraction
...
def credit_scenario(
    ...
    baseline = total * baseline_pct / 100
    additional = max(total - baseline, 0)
    leakage = additional * leakage_pct / 100
    buffer = additional * buffer_pct / 100
    uncertainty = additional * uncertainty_pct / 100
    ...
    return {
        ...
        "elegivel": eligible,
        "receita_usd": eligible * price_usd,
        "receita_brl": eligible * price_usd * usd_brl,
    }
